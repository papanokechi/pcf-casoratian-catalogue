...
\date{June 2026 \quad(\texttt{pcf-casoratian-catalogue} v0.1.0 draft)}
...
in the constant-coefficient case~\cite{mathlib}. We have not yet formalized it in the
present uniform-in-$k$, time-varying setting.
...
\emph{general-order, uniform-in-$k$} Casoratian Abel formula with time-varying
coefficients --- precisely the ``something more general'' the Ap\'ery formalization
lacked --- which is what the present note provides and machine-checks.
...
Mathlib \texttt{rev=v4.30.0}). The independent symbolic gate is
