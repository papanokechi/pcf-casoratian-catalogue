...
\date{June 2026 \quad(\texttt{pcf-casoratian-catalogue} v0.1.3)}
...
less automatic, and two \emph{distinct} hypotheses enter: one needs $c_0$ to be a unit
(so the recurrence is reversible and its solution set is a free $R$-module of rank $k$,
pinned by any $k$ consecutive values), and one needs $\Caso(n_0)$ to be a unit (so the
chosen solutions $s^{(0)},\dots,s^{(k-1)}$ actually span that module at the window
$n_0$). The two interact through the step law~\eqref{eq:step} --- with $c_0$ a unit at
every step, $\Caso$ is a unit at one window iff at all --- but neither implies the
other. This is exactly why we record the converse here as \textsc{structural} rather
than asserting it in that generality. We have not
